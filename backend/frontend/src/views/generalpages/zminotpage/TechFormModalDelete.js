...
const TechFormModalDelete = (props) => {
  ...
  const loadsystemsonZ = async () => {
        await axios.get(`http://localhost:8000/api/systemsonzbycarnumber/${props.carnumber}`)
          .then(response2 => {
            let tempsystemsonZ = response2.data;
            for(let i=0;i<tempsystemsonZ.length;i++){
            if (tempsystemsonZ[i].latest_recalibration_date)
              tempsystemsonZ[i].latest_recalibration_date = tempsystemsonZ[i].latest_recalibration_date.slice(0, 10);
            }
  ...
  async function DeleteCarDatasUnits() {
    ...
    for(let i=0;i<systemsonZ.length;i++){
      if(i == props.index){
        ...
        await axios.delete(`http://localhost:8000/api/systemsonz/${tempsystemonZId}`);
      }
    }
      ...
      <ModalBody>
        <Card>
          <CardHeader style={{ direction: 'rtl' }}>
            <CardTitle tag="h4" style={{ direction: 'rtl', textAlign: 'center', fontWeight: "bold" }}>מחיקת מערכת</CardTitle>{/*headline*/}
          </CardHeader>
          <CardBody style={{ direction: 'rtl' }}>
            <Container>
              <div style={{ textAlign: 'center', paddingTop: '20px' }}>
                ...
                <button className="btn-new-delete" onClick={() => {props.ToggleForModal()}}>בטל</button>
