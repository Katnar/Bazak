...
function ModularChartPage(props) {
	...
	const [chartidformodal, setChartidformodal] = useState(
	...
	const searchOrder = (evt) => {
		...
		setFilteredsubcharts(
			subcharts.filter((chart) =>
	...
	async function getchartdata() {
		...
		let response = await axios.get(
			`http://localhost:8000/api/modularscreens/chartbychartid/${props.match.params.chartid}`
		);
		...
		if (props.match.params.cartype == "notype") {
			if (tempchart.tenetree.length > 0) {
				for (let i = 0; i < tempchart.tenetree.length; i++) {
					for (
						let j = 0;
						j <
						tempchart.tenetree[i][Object.keys([tempchart.tenetree[i]][0]).pop()]
							.length;
						j++
					) {
						let tempsubchart = {};
						...
						let response = await axios.get(
							`http://localhost:8000/api/${lastKey}/${lastValue}`
						);
						tempsubchart.name = response.data.name;
						tempsubchart.tenetree = {
							[Object.keys([tempchart.tenetree[i]][0]).pop()]:
								response.data._id,
						};
			...
			if (props.match.params.cartype == "magad") {
				await axios
					.get(
						`http://localhost:8000/api/magad/magadsbymagadal/${props.match.params.carid}`
					)
					.then((response) => {
						for (let i = 0; i < response.data.length; i++) {
							let tempsubchart = {};
			...
			if (props.match.params.cartype == "mkabaz") {
				await axios
					.get(
						`http://localhost:8000/api/mkabaz/mkabazsbymagad/${props.match.params.carid}`
					)
					.then((response) => {
						for (let i = 0; i < response.data.length; i++) {
							let tempsubchart = {};
			...
			if (props.match.params.cartype == "makat") {
				await axios
					.get(
						`http://localhost:8000/api/makat/makatsbymkabaz/${props.match.params.carid}`
					)
					.then((response) => {
						for (let i = 0; i < response.data.length; i++) {
							let tempsubchart = {};
					...
					<div style={{ textAlign: "center", marginBottom: "20px" }}>
						{props.theme == "white-content" ? (
							<h1 style={{ fontWeight: "bold", color: "rgb(54,78,104)" }}>
								זמינות האמל"ח - {chartdata.name}
							</h1>
						) : (
							<h1 style={{ fontWeight: "bold", color: "hsla(0,0%,100%,.8)" }}>
								כשירות האמל"ח - {chartdata.name}
							</h1>
						)}
					</div>
