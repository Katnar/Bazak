...
function ModularScreenPage(props) {
	...
	const handleDragEnd = async () => {
		...
		let response2 = await axios
			.put(
				`http://localhost:8000/api/modularscreens/chart/${
					charts[targetDragChart.current].chartid
				}`,
				charts[targetDragChart.current]
			)
			.then(async (response2) => {
				let response3 = await axios
					.put(
						`http://localhost:8000/api/modularscreens/chart/${
							charts[dragChart.current].chartid
						}`,
						charts[dragChart.current]
					)
					.then((response3) => {});
			});
						...
						chart ? (
							mode == "edit" ? (
								<div
									...
									style={{ width: `${100 / screendata.chartsinline}%` }}
								>
									<ChartCard
										chart={chart}
										theme={props.theme}
										...
										init={() => init()}
										Toggle={() => Togglechartmodal(chart.chartid)}
