...
import {
	getCarDataFunc,
	findcardatabyidandupdateFunc,
	findcardatabyidanddeleteFunc,
} from "redux/features/cardata/cardataSlice";
...
const SortingTable = (props) => {
		...
		if (cardataidformodal != undefined) {
			if (props.unittype != "notype") {
				...
				let response = await axios.get(
					`http://localhost:8000/api/cardata/${cardataidformodal}`
				);
	...
	const getCardDataByUnitTypeAndUnitId = async () => {
		if (props.unittype != "notype") {
			...
			await axios
				.get(`http://localhost:8000/api/cardatanotype`)
				.then((response) => {
		...
		for (let k = 0; k < props.charts.length; k++) {
			...
			if (props.charts[k].units && props.charts[k].units.length > 0) {
				...
				for (let i = 0; i < props.charts[k].units.length; i++) {
					...
					let lastValue =
						props.charts[k].units[i][
							Object.keys(props.charts[k].units[i]).pop()
						];
					let temp = [];
					for (let j = 0; j < lastValue.length; j++) {
						temp = temp.concat(
							temp_cardata_by_chart.filter(
								(cardata) => cardata[lastKey] == lastValue[j]
							)
						);
					}
					temp_cardata_by_chart_copy3 =
						temp_cardata_by_chart_copy3.concat(temp); //theres duplicates
				}
			...
			if (props.charts[k].tenetree && props.charts[k].tenetree.length > 0) {
				...
				for (let i = 0; i < props.charts[k].tenetree.length; i++) {
					...
					let lastValue =
						props.charts[k].tenetree[i][
							Object.keys(props.charts[k].tenetree[i]).pop()
						];
					let temp = [];
					for (let j = 0; j < lastValue.length; j++) {
						temp = temp.concat(
					...
					temp_cardata_by_chart_copy4 =
						temp_cardata_by_chart_copy4.concat(temp); //theres duplicates
				}
			...
			//when drilling down
				...
				if (props.charts[k].tenetree) {
					...
					let lastValue =
						props.charts[k].tenetree[
							Object.keys(props.charts[k].tenetree).pop()
						];
					console.log("lastvalue: " + lastValue);
					let temp = temp_cardata_by_chart.filter(
						(cardata) => cardata[lastKey] == lastValue
					);
					console.log("temp: ");
					console.log(temp);
					temp_cardata_by_chart_copy4 =
			...
			for (let i = 0; i < props.charts[k].stand.length; i++) {
				let temp = temp_cardata_by_chart.filter(
					(cardata) => cardata.stand == props.charts[k].stand[i]
				);
			...
			for (let i = 0; i < props.charts[k].status.length; i++) {
				let temp = temp_cardata_by_chart.filter(
					(cardata) => cardata.status == props.charts[k].status[i]
				);
				temp_cardata_by_chart_copy2 = temp_cardata_by_chart_copy2.concat(temp);
			}
			temp_cardata_by_chart = temp_cardata_by_chart_copy2;
			//
			temp_cardata_by_chart_final = temp_cardata_by_chart_final.concat(
				temp_cardata_by_chart
			);
		}
	...
	const fixfilterbyurl = async () => {
		...
		if (props.unittype == "pikod") {
			...
			let response = await axios.get(
				`http://localhost:8000/api/ogda/${tempfilter.ogda}`
			);
			tempfilter.pikod = [response.data.pikod];
			...
			let response1 = await axios.get(
				`http://localhost:8000/api/hativa/${tempfilter.hativa}`
			);
			tempfilter.ogda = [response1.data.ogda];
			let response = await axios.get(
				`http://localhost:8000/api/ogda/${tempfilter.ogda}`
		...
		if (props.cartype == "magadal") {
			...
			let response1 = await axios.get(
				`http://localhost:8000/api/magad/${props.carid}`
			);
	...
	const applyfiltersontodata = () => {
		...
		if (filter.magadal && filter.magadal.length > 0) {
			myArrayFiltered7 = myArrayFiltered6.filter((item) =>
				filter.magadal.includes(item.magadal)
			);
		} else {
		...
		if (filter.magad && filter.magad.length > 0) {
			myArrayFiltered8 = myArrayFiltered7.filter((item) =>
				filter.magad.includes(item.magad)
			);
		} else {
		...
		if (filter.mkabaz && filter.mkabaz.length > 0) {
			myArrayFiltered9 = myArrayFiltered8.filter((item) =>
				filter.mkabaz.includes(item.mkabaz)
			);
		} else {
		...
		if (filter.makat && filter.makat.length > 0) {
			myArrayFiltered10 = myArrayFiltered9.filter((item) =>
				filter.makat.includes(item.makat)
			);
		} else {
	...
	function FixDataAndExportToExcel() {
			...
			if (data[i].tipuls.length != 0) {
				for (let j = 0; j < data[i].tipuls.length; j++) {
					...
					if (tempcardata.tipuls[j].type == "tipul") {
						...
						tempcardata.tipul_entry_date = tempcardata.tipuls[j]
							.tipul_entry_date
							? tempcardata.tipuls[j].tipul_entry_date
									.split("-")
									.reverse()
									.join("-")
							: null;
						...
						if (tempcardata.tipuls[j].hh_stands) {
							...
							for (let x = 0; x < tempcardata.tipuls[j].hh_stands.length; x++) {
								tempcardata.missing_makat_1[x] =
									tempcardata.tipuls[j].hh_stands[x].missing_makat_1;
								tempcardata.missing_makat_2[x] =
									tempcardata.tipuls[j].hh_stands[x].missing_makat_2;
							}
							tempcardata.missing_makat_1 =
								tempcardata.missing_makat_1.toString();
							tempcardata.missing_makat_2 =
								tempcardata.missing_makat_2.toString();
						}
					} else if (tempcardata.tipuls[j].type == "harig_tipul") {
						tempcardata.harig_tipul = tempcardata.tipuls[j].harig_tipul;
						tempcardata.harig_tipul_date = tempcardata.tipuls[j]
							.harig_tipul_date
							? tempcardata.tipuls[j].harig_tipul_date
									.split("-")
									.reverse()
									.join("-")
							: null;
						...
						tempcardata.takala_mizdamenet =
							tempcardata.tipuls[j].takala_mizdamenet;
						tempcardata.takala_mizdamenet_date =
							tempcardata.tipuls[j].takala_mizdamenet_date;
						if (tempcardata.tipuls[j].hh_stands) {
							tempcardata.missing_makat_1 = [];
							tempcardata.missing_makat_2 = [];
							for (let x = 0; x < tempcardata.tipuls[j].hh_stands.length; x++) {
		...
		for (let i = 0; i < tempdata_to_excel.length; i++) {
			tempdata_to_excel[i].pikod_data
				? (tempdata_to_excel[i].pikod_name =
						tempdata_to_excel[i].pikod_data[0].name)
				: (tempdata_to_excel[i].pikod_name = " ");
			tempdata_to_excel[i].ogda_data
				? (tempdata_to_excel[i].ogda_name =
						tempdata_to_excel[i].ogda_data[0].name)
				: (tempdata_to_excel[i].ogda_name = " ");
			tempdata_to_excel[i].hativa_data
				? (tempdata_to_excel[i].hativa_name =
						tempdata_to_excel[i].hativa_data[0].name)
				: (tempdata_to_excel[i].hativa_name = " ");
			tempdata_to_excel[i].gdod_data
				? (tempdata_to_excel[i].gdod_name = tempdata_to_excel[i].gdod_data.name)
				: (tempdata_to_excel[i].gdod_name = " ");

			tempdata_to_excel[i].magadal_data
				? (tempdata_to_excel[i].magadal_name =
						tempdata_to_excel[i].magadal_data[0].name)
				: (tempdata_to_excel[i].magadal_name = " ");
			tempdata_to_excel[i].magad_data
				? (tempdata_to_excel[i].magad_name =
						tempdata_to_excel[i].magad_data[0].name)
				: (tempdata_to_excel[i].magad_name = " ");
			tempdata_to_excel[i].mkabaz_data
				? (tempdata_to_excel[i].mkabaz_name =
						tempdata_to_excel[i].mkabaz_data[0].name)
				: (tempdata_to_excel[i].mkabaz_name = " ");
			tempdata_to_excel[i].makat_data
				? (tempdata_to_excel[i].makat_name =
						tempdata_to_excel[i].makat_data._id)
				: (tempdata_to_excel[i].makat_name = " ");
			tempdata_to_excel[i].makat_data
				? (tempdata_to_excel[i].makat_description_name =
						tempdata_to_excel[i].makat_data.name)
				: (tempdata_to_excel[i].makat_description_name = " ");

			tempdata_to_excel[i].latest_recalibration_date = tempdata_to_excel[i]
				.latest_recalibration_date
				? tempdata_to_excel[i].latest_recalibration_date
						.slice(0, 10)
						.split("-")
						.reverse()
						.join("-")
				: null;
			tempdata_to_excel[i].expected_repair = tempdata_to_excel[i]
				.expected_repair
				? tempdata_to_excel[i].expected_repair
						.slice(0, 10)
						.split("-")
						.reverse()
						.join("-")
			...
			tempdata_to_excel[i].updatedAt_data = tempdata_to_excel[i].updatedAt
		...
		for (let i = 0; i < tempdata_to_excel.length; i++) {
			...
			if (!tempdata_to_excel[i].carnumber) {
				tempdata_to_excel[i].carnumber = " ";
			}
			if (!tempdata_to_excel[i].expected_repair) {
				tempdata_to_excel[i].expected_repair = " ";
			}
			if (!tempdata_to_excel[i].family) {
				tempdata_to_excel[i].family = " ";
			}
			if (!tempdata_to_excel[i].gdod_name) {
				tempdata_to_excel[i].gdod_name = " ";
			}
			if (!tempdata_to_excel[i].hativa_name) {
				tempdata_to_excel[i].hativa_name = " ";
			}
			if (!tempdata_to_excel[i].kshirot) {
				tempdata_to_excel[i].kshirot = " ";
			}
			if (!tempdata_to_excel[i].latest_recalibration_date) {
				tempdata_to_excel[i].latest_recalibration_date = " ";
			}
			if (!tempdata_to_excel[i].magad_name) {
				tempdata_to_excel[i].magad_name = " ";
			}
			if (!tempdata_to_excel[i].magadal_name) {
				tempdata_to_excel[i].magadal_name = " ";
			}
			if (!tempdata_to_excel[i].makat_description_name) {
				tempdata_to_excel[i].makat_description_name = " ";
			}
			if (!tempdata_to_excel[i].makat_name) {
				tempdata_to_excel[i].makat_name = " ";
			}
			if (!tempdata_to_excel[i].mikum) {
				tempdata_to_excel[i].mikum = " ";
			}
			if (!tempdata_to_excel[i].mikum_bimh) {
				tempdata_to_excel[i].mikum_bimh = " ";
			}
			if (!tempdata_to_excel[i].mkabaz_name) {
				tempdata_to_excel[i].mkabaz_name = " ";
			}
			if (!tempdata_to_excel[i].ogda_name) {
				tempdata_to_excel[i].ogda_name = " ";
			}
			if (!tempdata_to_excel[i].pikod_name) {
				tempdata_to_excel[i].pikod_name = " ";
			}
			if (!tempdata_to_excel[i].pluga) {
				tempdata_to_excel[i].pluga = " ";
			}
			if (!tempdata_to_excel[i].shabzak) {
				tempdata_to_excel[i].shabzak = " ";
			}
			if (!tempdata_to_excel[i].stand) {
				tempdata_to_excel[i].stand = " ";
			}
			if (!tempdata_to_excel[i].status) {
				tempdata_to_excel[i].status = " ";
			}
			if (!tempdata_to_excel[i].takala_info) {
				tempdata_to_excel[i].takala_info = " ";
			}
			if (!tempdata_to_excel[i].zminot) {
				tempdata_to_excel[i].zminot = " ";
			} //22
			//
			if (!tempdata_to_excel[i].tipul) {
				tempdata_to_excel[i].tipul = " ";
			}
			if (!tempdata_to_excel[i].tipul_entry_date) {
				tempdata_to_excel[i].tipul_entry_date = " ";
			}
			if (!tempdata_to_excel[i].mikum_tipul) {
				tempdata_to_excel[i].mikum_tipul = " ";
			}
			if (!tempdata_to_excel[i].harig_tipul) {
				tempdata_to_excel[i].harig_tipul = " ";
			}
			if (!tempdata_to_excel[i].harig_tipul_date) {
				tempdata_to_excel[i].harig_tipul_date = " ";
			}
			if (!tempdata_to_excel[i].takala_mizdamenet) {
				tempdata_to_excel[i].takala_mizdamenet = " ";
			}
			if (!tempdata_to_excel[i].takala_mizdamenet_date) {
				tempdata_to_excel[i].takala_mizdamenet_date = " ";
			}
			if (!tempdata_to_excel[i].missing_makat_1) {
				tempdata_to_excel[i].missing_makat_1 = " ";
			}
			if (!tempdata_to_excel[i].missing_makat_2) {
				tempdata_to_excel[i].missing_makat_2 = " ";
			}
		...
		const headers = {
			carnumber: "צ'",
			magadal_name: "מאגד על",
			magad_name: "מאגד",
			mkabaz_name: "מקבץ",
			makat_name: 'מק"ט',
			makat_description_name: 'תיאור מק"ט',
			family: "משפחה",
			pikod_name: "פיקוד",
			ogda_name: "אוגדה",
			hativa_name: "חטיבה",
			...
			pluga: "פלוגה",
			shabzak: 'שבצ"ק',
			mikum_bimh: 'מיקום בימ"ח',
			stand: "מעמד",
			status: "סטאטוס",
			zminot: "זמינות",
			kshirot: "כשירות",
			mikum: "מיקום",
			latest_recalibration_date: "מועד כיול אחרון",
			takala_info: "מידע תקלה",
			expected_repair: "צפי תיקון",
			tipul: "טיפול",
			tipul_entry_date: "תאריך כניסה לטיפול",
			mikum_tipul: "מיקום טיפול",
			harig_tipul: "חריג טיפול",
			harig_tipul_date: "תאריך חריגת טיפול",
			takala_mizdamenet: "תקלה מזדמנת",
			takala_mizdamenet_date: "תאריך תקלה מזדמנת",
	...
	const {
	...
	} = useTable(
		{
			...
			initialState: {
				pageIndex: 0,
				hiddenColumns: localStorage.getItem("zminot_page_hidden_columns")
					? JSON.parse(localStorage.getItem("zminot_page_hidden_columns"))
					: [],
			},
		},
		useGlobalFilter,
		...
		<>
			...
			>
				...
				<CarDataFilter
					...
					carid={props.carid}
					/*handleChange2={handleChange2}*/ allColumns={allColumns}
					handleChange8={handleChange8}
					handleChangeForTaarichTipul={handleChangeForTaarichTipul}
				/>

				<div style={{ float: "right", paddingBottom: "5px" }}>
					<button className="btn-new-blue" onClick={FixDataAndExportToExcel}>
						הורד כקובץ אקסל
					</button>
				</div>
								...
								<tr className={styles.tr} {...row.getRowProps()}>
									{row.cells.map((cell) => {
										if (
											...
											cell.column.id != "latest_recalibration_date" &&
											cell.column.id != "pikod" &&
											cell.column.id != "ogda" &&
											cell.column.id != "hativa" &&
											cell.column.id != "gdod" &&
											cell.column.id != "magadal" &&
											cell.column.id != "magad" &&
											cell.column.id != "mkabaz" &&
											...
											if (cell.column.id == "pikod") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.pikod_data
															? row.original.pikod_data[0].name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "ogda") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.ogda_data
															? row.original.ogda_data[0].name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "hativa") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.hativa_data
															? row.original.hativa_data[0].name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "gdod") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.gdod_data
															? row.original.gdod_data.name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "magadal") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.magadal_data
															? row.original.magadal_data[0].name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "magad") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.magad_data
															? row.original.magad_data[0].name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "mkabaz") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.mkabaz_data
															? row.original.mkabaz_data[0].name
															: null}
													</td>
												) : (
													<td
											...
											if (cell.column.id == "makat") {
												return cell.value ? (
													<td
														...
														{...cell.getCellProps()}
													>
														{row.original.makat_data
															? row.original.makat_data._id
															: null}
													</td>
												) : (
													<td
													...
													>
														{cell.value
															.filter(
																(value, index, self) =>
																	index ===
																	self.findIndex((t) => t.type === value.type)
															)
															.map((tipul, index) =>
																tipul.type == "tipul" ? (
																	<p>טיפול</p>
																) : tipul.type == "harig_tipul" ? (
																	<p>חריג טיפול</p>
																) : tipul.type == "takala_mizdamenet" ? (
																	<p>תקלה מזדמנת</p>
																) : tipul.type == "hh_stand" ? (
																	<p>עומד על ח"ח</p>
																) : (
