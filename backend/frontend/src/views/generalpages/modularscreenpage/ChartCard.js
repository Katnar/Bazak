...
const ChartCard = (props) => {
	...
	const [cardata_by_chart_harigtipul, setCardata_by_chart_harigtipul] =
		useState(0);
	const [
		cardata_by_chart_takalotmizdamnot,
		setCardata_by_chart_takalotmizdamnot,
	] = useState(0);
	const [
		cardata_by_chart_hhstand_intipul,
		setCardata_by_chart_hhstand_intipul,
	] = useState(0);
	const [
		cardata_by_chart_hhstand_harigtipul,
		setCardata_by_chart_hhstand_harigtipul,
	] = useState(0);
	const [
		cardata_by_chart_hhstand_takalotmizdamnot,
		setCardata_by_chart_hhstand_takalotmizdamnot,
	] = useState(0);
	//sysonz
	const [
		cardata_by_chart_system_mooshbat,
		setCardata_by_chart_system_mooshbat,
	] = useState(0);
	const [cardata_by_chart_systemonz, setCardata_by_chart_systemonz] =
		useState(0);
	const [cardata_by_chart_systemonz_hh, setCardata_by_chart_systemonz_hh] =
		useState(0);
	//
	...
	const clickDelete = async () => {
		...
		for (let i = 0; i < props.charts.length; i++) {
			...
			if (flag) {
				...
				let response2 = await axios
					.put(
						`http://localhost:8000/api/modularscreens/chart/${props.charts[i].chartid}`,
						props.charts[i]
					)
					.then((response2) => {});
			}
		...
		let response = axios.post(
			`http://localhost:8000/api/modularscreens/chart/remove/${props.chartid}`
		);
	...
	function init() {
		...
		if (props.chart.units && props.chart.units.length > 0) {
			...
			for (let i = 0; i < props.chart.units.length; i++) {
				...
				let lastValue =
					props.chart.units[i][Object.keys(props.chart.units[i]).pop()];
				let temp = [];
		...
		if (props.chart.tenetree && props.chart.tenetree.length > 0) {
			...
			for (let i = 0; i < props.chart.tenetree.length; i++) {
				...
				let lastValue =
					props.chart.tenetree[i][Object.keys(props.chart.tenetree[i]).pop()];
				let temp = [];
				for (let j = 0; j < lastValue.length; j++) {
					temp = temp.concat(
						temp_cardata_by_chart.filter(
		...
		for (let i = 0; i < props.chart.stand.length; i++) {
			let temp = temp_cardata_by_chart.filter(
				(cardata) => cardata.stand == props.chart.stand[i]
			);
		...
		for (let i = 0; i < props.chart.status.length; i++) {
			let temp = temp_cardata_by_chart.filter(
				(cardata) => cardata.status == props.chart.status[i]
			);
		...
		if (props.theme == "white-content") {
			temp_cardata_by_chart_instate = temp_cardata_by_chart.filter(
				(cardata) => cardata.zminot == "זמין"
			);
			temp_cardata_by_chart_not_instate = temp_cardata_by_chart.filter(
				(cardata) => cardata.zminot != "זמין"
			);
		} else {
			temp_cardata_by_chart_instate = temp_cardata_by_chart.filter(
		...
		let temp_cardata_by_chart_systemonz = [];
		let temp_cardata_by_chart_systemonz_mooshbat = 0;
		...
		for (let i = 0; i < temp_cardata_by_chart.length; i++) {
			let is_system_hh_multival = false;
		    const sys = props.systemsonz;
			sys.map((item) => {
				if (
					item.carnumber == temp_cardata_by_chart[i].carnumber
				) {
						temp_cardata_by_chart_systemonz.push(
							temp_cardata_by_chart[i]
						);
				}
			});
			unique = [
				...new Map(
					temp_cardata_by_chart_systemonz.map((m) => [m.carnumber, m])
				).values(),
			];
			const fl = sys.filter(
				(item) =>
					item.carnumber == temp_cardata_by_chart[i].carnumber && item.kshirot == "לא כשיר"
			);
			unique1 = [...new Map(fl.map((m) => [m.carnumber, m])).values()];
			temp_cardata_by_chart_systemonz_mooshbat = temp_cardata_by_chart_systemonz_mooshbat + unique1.length;
			sys.map((item) => {
				try {
					if (
						item.carnumber == temp_cardata_by_chart[i].carnumber
		...
		setCardata_by_chart_systemonz(unique.length);
		setCardata_by_chart_system_mooshbat(
			temp_cardata_by_chart_systemonz_mooshbat
		);
		setCardata_by_chart_systemonz_hh(
			!temp_system_hh
				? 0
	...
	return props.mode == "normal" ? (
			...
			<div style={{ width: `100%`, paddingLeft: "15px", paddingRight: "15px" }}>
				<Card style={{ boxShadow: "rgb(123 123 123 / 20%) 0px 2px 5px 5px" }}>
							...
							{props.theme == "white-content" ? (
								<>
									<Link
										...
										to={`/modularchartpage/${props.chart.chartid}/notype/0`}
									>
										<img
					...
					>
						...
						>
							{(cardata_by_chart != 0
								? (cardata_by_chart_instate / cardata_by_chart) * 100
								: 0) <= props.chart.redcolor ? (
								<ProgressProvider
									valueStart={0}
									valueEnd={
										cardata_by_chart != 0
									...
									{(value) => (
										...
										>
											...
											<div style={{ fontSize: 12, marginTop: -2 }}>
												<h5 style={{ margin: "0px" }}>
													{cardata_by_chart_instate + "/" + cardata_by_chart}
												</h5>
											</div>
										</CircularProgressbarWithChildren>
									)}
						...
						<div style={{ display: "inline-flex", marginTop: "10px" }}>
							...
							{props.chart.yellowcolor != 0 ? (
								<>
									...
									/>
									<p>
										{props.chart.redcolor}-{props.chart.yellowcolor}
									</p>
								</>
							) : null}
						...
						<div
							style={{
								textAlign: "left",
								marginTop: `-${1.5 * (props.chart.stand.length + 1)}rem`,
								marginBottom: "1rem",
							}}
						>
							{props.chart.stand.map((stand) => (
								<h4 style={{ marginBottom: "0px" }}>{stand}</h4>
							))}
						</div>
						...
						{collapseOpen ? (
							...
							>
								...
								{console.log(cardata_by_chart_system_mooshbat,cardata_by_chart_systemonz )}
								<h6>
									{props.chart.name} אי כשירות מערכת:{" "}
									{cardata_by_chart_system_mooshbat}{" "}
									<span style={{ color: "DarkTurquoise" }}>
										(חלפים: {cardata_by_chart_systemonz_hh})
									</span>
								</h6>
								<Progress
									color="guyblue"
									value={
										cardata_by_chart_systemonz != 0
											? (cardata_by_chart_system_mooshbat /
													cardata_by_chart_systemonz) *
											  100
											: 0
									}
									style={{ height: "10px", marginBottom: "8px" }}
		...
		<div style={{ width: `100%`, paddingLeft: "15px", paddingRight: "15px" }}>
			<Card style={{ boxShadow: "rgb(123 123 123 / 20%) 0px 2px 5px 5px" }}>
				<CardHeader style={{ padding: "0px" }}>
					<div
						style={{ textAlign: "left", height: "0px", marginRight: "3rem" }}
					>
						<button
